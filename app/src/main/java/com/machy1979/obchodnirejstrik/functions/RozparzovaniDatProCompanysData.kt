...
class RozparzovaniDatProCompanysData {
    companion object {
        fun vratCompanyData(element: Element): CompanyData {
            ...
            companyData.name = element.select("are|Obchodni_firma")?.text()
                ?: " " //pokud element nenajde (tím pádem bude null), tak se propíše " " - aneb Elvis...
...
class RozparzovaniDatProCompanysDataNovy {
    companion object {
        fun vratCompanyData(jsonObject: JSONObject): CompanyData {
            val companyData = CompanyData()
            companyData.name = jsonObject.optString("obchodniJmeno", " ")
                ?: " " //optString je lepší, než getString, protože pokud obchodniJmeno neexistuje, nahradí to fallbackem
