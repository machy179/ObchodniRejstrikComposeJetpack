...
class RozparzovaniDatDotazRZP {
    companion object {
        ...
        fun vratCompanyData(jsonObject: JSONObject, context: Context): CompanyDataRZP {
            ...
            //evdidujici úřad
            val evidujiciUradZnacka = jsonObject.optString("zivnostenskyUrad", " ")
                ?: " " //musím ještě udělat číselník a namapovat to obdobně, jako právní forma
            val evidujiciUradyCiselnikArray =
                context.resources.getStringArray(R.array.zivnostensky_urad)
            val evidujiciUradyCiselnikMap =
                evidujiciUradyCiselnikArray.map { it.split(",") }.associate { it[0] to it[1] }
            val evidujiciUrad =
                evidujiciUradyCiselnikMap[evidujiciUradZnacka] ?: evidujiciUradZnacka
            ...
            val companyDataRZP = CompanyDataRZP(
                name, ico, "", address, pravniForma, typSubjektu, evidujiciUrad, vznikPrvniZivnosti,
