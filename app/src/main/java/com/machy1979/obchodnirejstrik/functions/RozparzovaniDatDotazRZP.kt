...
class RozparzovaniDatDotazRZP {
    companion object {
        fun vratCompanyData(document: Document): CompanyDataRZP {
            Log.i("chybaaa2: ",document.toString())
            val name = document.select("D|OF").first()?.text() ?: " "
            val ico = document.select("D|ICO").first()?.text() ?: " "
            var address =  vratAdresu(document.select("D|A")) ?: " "

//            val stavSubjektu= document.select("D|SSU").first()?.text() ?: " "
            val pravniForma= document.select("D|NPF").first()?.text() ?: " "
            val typSubjektu= document.select("D|PF").select("D|T").first()?.text() ?: " "
            val evidujiciUrad= document.select("D|ZU").select("D|NZU").first()?.text() ?: " "
            val vznikPrvniZivnosti= document.select("D|ZI1").first()?.text() ?: " "
